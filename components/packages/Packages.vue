<template>
  <section-container class="mb-56">
      ...
      >
        <li v-for="item in data" :key="item.title">
          <a
            ...
            :class="[
              getActiveServices[0].title === item.title
                ? 'text-app-green-1'
                : '',
            ]"
    ...
    <div
      v-if="getActiveServices != null"
      class="flex justify-center space-x-1 shadow-2xl"
    >
      <div
        v-for="(item, index) in getActiveServices[0].packages"
        :key="item.title"
        :class="[
          index === 0 ? 'rounded-l-2xl' : '',
          index === getActiveServices[0].packages.length - 1
            ? 'rounded-r-2xl'
            : '',
        ]"
        class="w-3/12 bg-app-gray-2 flex flex-col text-center items-center justify-between px-8 2xl:px-10 py-8"
      >
        ...
        <div>
          <div v-for="obj in item.details" :key="obj.value" class="">
