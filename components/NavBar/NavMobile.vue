      ...
      <ul class="flex flex-col h-full items-center justify-center align-center">
        <li v-for="link in navLinks" :key="link.name" class="my-3">
          <a
            v-if="!link.button"
            :href="link.href"
            class="text-lg font-bold hover:text-app-green-1 transition duration-200"
          >
            {{ link.name }}
          </a>
          <button
            v-else
            class="px-5 py-1 text-lg text-app-gray-1 font-bold rounded-full bg-app-green-1 hover:text-app-green-1 hover:bg-app-gray-1 border-2 border-app-green-1 transition-all ease-in-out duration-200 focus:outline-none"
          >
